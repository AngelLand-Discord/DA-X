...
def staff_access(guild_id, level, guild):
    ...
    if request.method == "POST":
        user_id = clean_text(request.form.get("user_id"), 25)
        if not valid_discord_id(user_id):
            db.close()
            return render_template("error.html", title="Invalid user ID", message="Enter a valid Discord user ID."), 400
        db.execute(
            "INSERT OR REPLACE INTO guild_permissions (guild_id, user_id, role, added_by, created_at) VALUES (?, ?, ?, ?, ?)",
            (guild_id, user_id, "staff", current_user_id(), utc_now()),
        )
        db.commit()
        db.close()
        return redirect(url_for("staff_access", guild_id=guild_id))
    staff_users = db.execute("SELECT * FROM guild_permissions WHERE guild_id=? ORDER BY user_id", (guild_id,)).fetchall()
    db.close()
    return render_template("staff_access.html", guild_id=guild_id, guild_name=guild["name"], staff_users=staff_users)
